...
from models.hat import HATConfig, HATTokenizer,  HATModelForLogsPreTraining, HATModelForSimCLRPreTraining
...
class DataTrainingArguments:
    ...
    mlm: int = field(default=40, metadata={"help": "MLM probability."})
    sr: int = field(default=20, metadata={"help": "Sentence replacement probability."})
    topk: int = field(default=3, metadata={"help": "Top-k for MLM accuracy."})
...
def main():
    ...
    def numpy_mask_tokens(inputs: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        inputs = np.array(inputs)
        mlm_probability = data_args.mlm / 100
    ...
    def preprocess_function(examples):
        ...
        for example_idx, input_ids in enumerate(batch['input_ids']):
            ...
            sentence_masks.append([True] * n_sentences + [False] * num_pad_sentences)

            sec_temp_input_ids = []

            # secondary
            for idx in secondary_positions:
                sec_temp_input_ids.extend(batch['input_ids'][example_idx][config.max_sentence_length * idx:config.max_sentence_length * (idx+1)])
            
            sec_temp_input_ids.extend([config.pad_token_id] * (config.max_sentence_length * num_pad_sentences))

            batch_secondary_input_ids.append(sec_temp_input_ids)
        ...
        batch['sentence_masks'] = sentence_masks
        batch['input_ids'], batch['labels'] = numpy_mask_tokens(batch_input_ids)
        batch['secondary_input_ids'], batch['secondary_labels'] = numpy_mask_tokens(batch_secondary_input_ids)
    ...
    def preprocess_logits_for_metrics(logits, labels):
            if isinstance(logits, tuple):
                ...
                mlm_logits = logits[7]
            mlm_indices = torch.topk(mlm_logits, k=data_args.topk, dim=-1)[1]
            return (mlm_indices,)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        mlm_labels = p.label_ids[0][:, :mlm_preds.shape[1]]
        logs_labels = p.label_ids[2]
        ...
        score = [1-top_k_list[i] for i in range(logs_labels.shape[0])]
