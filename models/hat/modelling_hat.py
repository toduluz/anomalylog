...
from torch.nn import BCEWithLogitsLoss, CrossEntropyLoss, MSELoss, CosineEmbeddingLoss, NLLLoss, LogSoftmax, TripletMarginLoss
...
@dataclass
class HATForLogsPreTrainingOutput(ModelOutput):
    """
    Output type of [`HATForLogsPreTraining`].
    ...
    Args:
        ...
        mlm_loss (*optional*, returned when `labels` is provided, `torch.FloatTensor` of shape `(1,)`):
        The masked language modeling loss.
        sent_order_loss (*optional*, returned when `labels` is provided, `torch.FloatTensor` of shape `(1,)`):
        The sentence order loss.
        primary_prediction_logits (`torch.FloatTensor` of shape `(batch_size, sequence_length, config.vocab_size)`):
            Prediction scores of the language modeling head (scores for each vocabulary token before SoftMax)
        primary_prediction_logits (`torch.FloatTensor` of shape `(batch_size, num_sentences, 1)`):
            Prediction scores of the sentence order.
        hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `torch.FloatTensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.
...
class HATConfig(PretrainedConfig):
    ...
    Args:
        ...
        max_sentence_length (:obj:`int`, `optional`, defaults to 128):
...
@add_start_docstrings(
    """
    HAT Model with two heads on top as done during the pretraining: a `masked language modeling` head and a `document
    segment order prediction ` head.
    """,
    HAT_START_DOCSTRING,
)
class HATModelForLogsPreTraining(HATPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.classifier_secondary = nn.Linear(config.max_sentences, config.num_labels)
        ...
        self.classifier_tertiary = nn.Linear(config.max_sentences, config.num_labels)
    ...
    def forward(
        ...
        secondary_prediction_scores = self.classifier_secondary(secondary_outputs.view(-1, self.max_sentences))
        ...
        tertiary_prediction_scores = self.classifier_tertiary(tertiary_outputs.view(-1, self.max_sentences))
        ...
        #         loss_fct = MSELoss()
        #         if self.num_labels == 1:
        #             sent_order_loss = loss_fct(secondary_prediction_scores.view(-1, 1).squeeze(), secondary_labels.view(-1).squeeze())
        #         else:
        #             sent_order_loss = loss_fct(secondary_prediction_scores.view(-1, 1), secondary_labels.view(-1))
        #     elif self.config.problem_type == "single_label_classification":
        #         loss_fct = CrossEntropyLoss()
        #         sent_order_loss = loss_fct(secondary_prediction_scores.view(-1, self.num_labels), secondary_labels.view(-1, self.num_labels))
        #     # elif self.config.problem_type == "multi_label_classification":
        #     #     loss_fct = BCEWithLogitsLoss()
        #     #     mask = secondary_labels[:, :, 0] != -1
        #     #     sent_order_loss = loss_fct(secondary_prediction_scores[mask], secondary_labels[mask])
        # if masked_lm_loss and sent_order_loss:
        #     total_loss = masked_lm_loss + sent_order_loss
        # elif masked_lm_loss:
