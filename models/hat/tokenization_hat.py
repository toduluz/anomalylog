    ...
    def __call__(self, text, **kwargs):
        ...
        if text_pair:
            ...
            for idx, sample in enumerate(batch['input_ids']):
                n_sentences = sum(sample[::self.config.max_sentence_length])
                for input_key in batch:
                    batch[input_key][idx][self.config.max_sentence_length * n_sentences:
                                          self.config.max_sentence_length * (n_sentences + 1)] = \
                        self.pad_sentence(batch_b[input_key][idx], chunk_size=self.config.max_sentence_length,
        ...
        for text_idx, text in enumerate(texts):
            ...
            for input_key in example_batch:
                ...
                for idx, example in enumerate(example_batch[input_key][:self.config.max_sentences]):
                    key_inputs_list.append(self.pad_sentence(example, chunk_size=self.config.max_sentence_length, special_id=self.type2id[input_key]))
